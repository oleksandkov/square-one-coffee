#' ---
#' title: "Ellis-5: Edmonton Open Data"
...
#+ echo=FALSE
# Rscript manipulation/ellis-5-open-data.R  # run from project root
...
API_BASE_URL <- "https://data.edmonton.ca/resource/eg3i-f4bj.csv"
OUTPUT_DIR <- "data-private/derived/ellis-5-open-data"
OUTPUT_CSV <- file.path(OUTPUT_DIR, "ellis-5-open-data.csv")
OUTPUT_RDS <- file.path(OUTPUT_DIR, "ellis-5-open-data.rds")
...
tryCatch({
  ...
  dbWriteTable(conn, "ellis_5_open_data", open_data, overwrite = TRUE)
  dbDisconnect(conn)
  
  message("Saved to SQLite: ", DB_PATH)
  message("  Table: ellis_5_open_data")
  message("  Records: ", nrow(open_data))
}, error = function(e) {
...
if (file.exists(OUTPUT_CSV) && file.exists(OUTPUT_RDS)) {
  message("✅ Ellis-5 completed successfully!")
